...
Select min(user_id) From users Group BY user_name Having count(*)>1 ) 
...
select * from 
(select * from employee ORDER BY emp_id DESC  LIMIT 4) AS t1 order by emp_id limit 1
...
JOIN (
    ...
    FROM employee
    GROUP BY dept_name
) t ON e.dept_name = t.dept_name
WHERE e.salary = t.min_salary
   OR e.salary = t.max_salary
